package dev.lazycat.sakuraLand.currency.sparks.commands;
...
public class SparksC {
    public static class SparksCommand implements CommandExecutor {
        private final SakuraLand plugin;
        private final MiniMessage mm = MiniMessage.miniMessage();
        public SparksCommand(SakuraLand plugin) {
            this.plugin = plugin;
        }
        @Override
        public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
            ...
            switch (action.toLowerCase()) {
                case "waste" -> {
                    ...
                    plugin.getSparksInstance().waste(player, value);
                    commandSender.sendMessage(mm.deserialize("<gold>Вы успешно потратили <value> искорок у игрока с никнеймом <player>"));
                    return true;
                }
                case "add" -> {
                    if (player.isEmpty() || value.isEmpty()) {
                    ...
                    plugin.getSparksInstance().add(player, value);
                    commandSender.sendMessage(mm.deserialize("<gold>Вы успешно дали <value> искорок игроку с никнеймом <player>"));
                    return true;
                }
                case "set" -> {
                    if (player.isEmpty() || value.isEmpty()) {
                    ...
                    plugin.getSparksInstance().set(player, value);
                    commandSender.sendMessage(mm.deserialize("<gold>Вы успешно поставили <value> искорок игроку с никнеймом <player>"));
                    return true;
                }
                default -> {
                    return false;
    ...
    public static class SparksCommandCompleter implements TabCompleter {
        @Override
